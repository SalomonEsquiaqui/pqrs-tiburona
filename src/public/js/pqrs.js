...
// ── RENDER MÓVIL: cards en lugar de tabla ──────────────────────────────────
// Se sobreescribe renderTabla para que en pantallas pequeñas use cards
...
function renderTabla(lista) {
  ...
  // ── VISTA MÓVIL: cards ──
  const tbody = document.getElementById('tabla-mis-pqrs');
  // Ocultar tabla padre y mostrar un contenedor de cards hermano
  ...
  cardsWrap.innerHTML = lista.map(p => `
    <div style="background:#fff;border-radius:14px;padding:16px;box-shadow:0 1px 4px rgba(15,23,42,.08);border:1px solid #e2e8f0;border-left:4px solid ${colores[p.estado]||'#e2e8f0'};">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;gap:8px;">
        <div>
          <code style="font-size:0.72rem;background:#f1f5f9;padding:2px 7px;border-radius:5px;color:#475569;font-weight:700;">${p.radicado}</code>
          <p style="font-weight:700;color:#0f172a;font-size:0.92rem;margin-top:6px;line-height:1.3;">${p.asunto}</p>
        </div>
        <span style="background:${colores[p.estado]||'#e2e8f0'}18;color:${colores[p.estado]||'#64748b'};border:1px solid ${colores[p.estado]||'#e2e8f0'}40;padding:3px 10px;border-radius:99px;font-size:0.7rem;font-weight:700;white-space:nowrap;flex-shrink:0;">${p.estado.replace('_',' ')}</span>
      </div>
      <div style="display:flex;align-items:center;gap:8px;font-size:0.78rem;color:#64748b;margin-bottom:12px;">
        <span>${iconos[p.tipo]||'📋'} ${p.tipo}</span>
        <span>·</span>
...
// Volver a la tabla si el usuario rota la pantalla a landscape
window.addEventListener('resize', () => {
  const cardsWrap = document.getElementById('cards-mobile-pqrs');
  const tablaWrap = document.querySelector('#sec-mis-pqrs .tabla-wrap');
  if (!cardsWrap || !tablaWrap) return;
  if (window.innerWidth > 768) {
    cardsWrap.style.display = 'none';
    tablaWrap.style.display = '';
    _renderTablaOriginal(todasMisPqrs);
  } else {
