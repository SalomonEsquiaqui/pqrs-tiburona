...
// ── RENDER MÓVIL ADMIN: cards en lugar de tabla ────────────────────────────
const _renderAdminOriginal = renderAdminPqrs;
function renderAdminPqrs(lista) {
  if (window.innerWidth > 768) {
    _renderAdminOriginal(lista);
    return;
  }
  const tbody    = document.getElementById('tabla-admin-pqrs');
  ...
  cardsWrap.innerHTML = lista.map(p => `
    <div style="background:#fff;border-radius:14px;padding:16px;box-shadow:0 1px 4px rgba(15,23,42,.08);border:1px solid #e2e8f0;border-left:4px solid ${colores[p.estado]||'#e2e8f0'};">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;gap:8px;">
        ...
        <span style="background:${colores[p.estado]||'#e2e8f0'}18;color:${colores[p.estado]||'#64748b'};border:1px solid ${colores[p.estado]||'#e2e8f0'}40;padding:3px 10px;border-radius:99px;font-size:0.68rem;font-weight:700;white-space:nowrap;flex-shrink:0;">${p.estado.replace('_',' ')}</span>
      </div>
      <div style="font-size:0.78rem;color:#64748b;margin-bottom:4px;">
        👤 ${p.users?.nombre||'—'} &nbsp;·&nbsp; ${iconos[p.tipo]||'📋'} ${p.tipo}
      </div>
      <div style="font-size:0.75rem;color:#94a3b8;margin-bottom:12px;">
        📅 ${formatFecha(p.created_at)} &nbsp;·&nbsp; 🏢 ${p.area||'—'}
      </div>
...
// Render tabla usuarios en móvil
const _cargarUsuariosOriginal = cargarUsuarios;
async function cargarUsuarios() {
  await _cargarUsuariosOriginal();
  if (window.innerWidth <= 768) _adaptarTablaUsuariosMobile();
}

function _adaptarTablaUsuariosMobile() {
  const tbody = document.getElementById('tabla-usuarios');
  if (!tbody) return;
  const tablaWrap = tbody.closest('.tabla-wrap');
  ...
  if (!filas.length || filas[0].textContent.includes('Cargando')) {
    cardsWrap.innerHTML = `<div style="text-align:center;padding:30px;background:#fff;border-radius:14px;color:#94a3b8;">Sin usuarios</div>`;
    return;
  }
  cardsWrap.innerHTML = filas.map(tr => {
    ...
    const tel    = celdas[2]?.textContent.trim();
    const rolBadge = celdas[3]?.innerHTML.trim();
    const select = celdas[4]?.innerHTML.trim();
    return `
      <div style="background:#fff;border-radius:14px;padding:16px;box-shadow:0 1px 4px rgba(15,23,42,.08);border:1px solid #e2e8f0;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
          <strong style="font-size:0.92rem;color:#0f172a;">${nombre}</strong>
          ${rolBadge}
        </div>
        <p style="font-size:0.8rem;color:#64748b;margin-bottom:2px;">✉️ ${email}</p>
        <p style="font-size:0.8rem;color:#94a3b8;margin-bottom:12px;">📞 ${tel||'—'}</p>
        <div style="display:flex;align-items:center;gap:8px;">
          <span style="font-size:0.75rem;color:#64748b;">Cambiar rol:</span>
          ${select}
        </div>
      </div>`;
  }).join('');
...
window.addEventListener('resize', () => {
  ['cards-mobile-admin','cards-mobile-usuarios'].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    el.style.display = window.innerWidth > 768 ? 'none' : 'flex';
  });
  if (window.innerWidth > 768) {
    document.querySelectorAll('#sec-todas-pqrs .tabla-wrap, #sec-usuarios .tabla-wrap').forEach(t => t.style.display = '');
    _renderAdminOriginal(todasPqrs);
  }
});
